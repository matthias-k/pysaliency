...
def test_COCO_Freeview(location):
    ...
    if location is None:
        ...
        assert isinstance(stimuli_test, pysaliency.Stimuli)
        assert not isinstance(stimuli_test, pysaliency.FileStimuli)
    else:
        ...
        assert location.join('COCO-Freeview/stimuli_test.hdf5').check()
...
@pytest.mark.download
@pytest.mark.nonfree
def test_COCO_Freeview_with_test(location):
    real_location = _location(location)

    stimuli_train, fixations_train, stimuli_val, fixations_val, stimuli_test = pysaliency.external_datasets.get_COCO_Freeview(location=real_location, test_data='ThirdParty/COCO/COCOFreeView_fixations_test_new.json')
    if location is None:
        assert isinstance(stimuli_train, pysaliency.Stimuli)
        assert not isinstance(stimuli_train, pysaliency.FileStimuli)
        assert isinstance(stimuli_val, pysaliency.Stimuli)
        ...
        assert location.join('COCO-Freeview/fixations_train.hdf5').check()
        assert location.join('COCO-Freeview/fixations_validation.hdf5').check()
        assert location.join('COCO-Freeview/fixations_test.hdf5').check()

    fixations_test = pysaliency.read_hdf5(str(location.join('COCO-Freeview/fixations_test.hdf5')))
    ...
    assert len(fixations_test.x) == 154154

    assert np.mean(fixations_test.x) == approx(853.7380314490705)
    assert np.mean(fixations_test.y) == approx(519.4896415273038)
    assert np.mean(fixations_test.t) == approx(7.594574256911919)
    assert np.mean(fixations_test.scanpath_history_length) == approx(7.594574256911919)
    assert np.mean(fixations_test.duration) == approx(0.2615992449109333)

    assert np.std(fixations_test.x) == approx(293.0185179686817)
    assert np.std(fixations_test.y) == approx(183.48201719100905)
    assert np.std(fixations_test.t) == approx(4.983095899305675)
    assert np.std(fixations_test.scanpath_history_length) == approx(4.983095899305675)
    assert np.std(fixations_test.duration) == approx(0.1788878832915657)

    assert kurtosis(fixations_test.x) == approx(-0.43932134352939967)
    assert kurtosis(fixations_test.y) == approx(-0.16783224862720658)
    assert kurtosis(fixations_test.t) == approx(-0.7842602163619645)
    assert kurtosis(fixations_test.scanpath_history_length) == approx(-0.7842602163619645)
    assert kurtosis(fixations_test.duration) == approx(122.74787622396316)


    assert skew(fixations_test.x) == approx(0.041065458982844354)
    assert skew(fixations_test.y) == approx(0.13852645233252112)
    assert skew(fixations_test.t) == approx(0.28877843053685415)
    assert skew(fixations_test.scanpath_history_length) == approx(0.28877843053685415)
    assert skew(fixations_test.duration) == approx(6.980496743557728)


    assert entropy(fixations_test.n) == approx(9.959947067374634)
    assert (fixations_test.n == 0).sum() == 154
